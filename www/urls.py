...
def make_signed_cookie(id, password, max_age):
    # build cookies string by: id-expires-md5
...
def manage_blogs_edit(blog_id):
    ...
    if blog is None:
        raise notfound()
    return dict(id=blog.id, name=blog.name, summary=blog.summary, conetnet=blog.content, action='/api/blogs/%s' % blog_id, redirect='/manage/blogs', user=ctx.request.user)
...
def api_get_comments():
    ...
    comments = Comment.find_by('order by created_at desc')
